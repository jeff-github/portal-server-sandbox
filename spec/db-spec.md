...
### 1. Audit Table (Immutable Event Log)
- **Insert-only** table - no updates or deletes allowed
- Records every data creation, modification, and deletion
- Each audit entry contains:
  - Auto-incrementing audit ID (primary key)
  - Event UUID (generated by mobile app)
  - Patient ID
  - Site ID
  - Full data snapshot or delta (JSONB)
  - Actor (user/investigator/admin who made change)
  - Timestamps (client-side and server-side)
  - Change reason (required)
  - Parent audit ID (for tracking change lineage)
- Serves as source of truth for regulatory compliance
- Enables point-in-time reconstruction of any record

### 2. State Table (Current View)
- One row per diary entry/event
- Can be updated (but all changes logged to audit table)
- Each row contains:
  ...
  - Sync metadata
- Derived from audit table via database triggers
- Optimized for fast queries by investigators
...
- **Database triggers** automatically create audit entries on all state table changes
- Trigger ensures atomic transaction: both audit and state update together
- Failed transactions rollback both tables
- No application code can bypass audit logging

### 4. State Derivation
- Trigger on audit table automatically updates state table
- Application cannot directly update state table (enforced by permissions)
- Ensures state table always reflects audit history
...
6. Database creates audit entry and updates state table
