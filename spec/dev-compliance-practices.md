...
> **See**: dev-core-practices.md for core development principles

**Version**: 2.1.0 | **Ratified**: [Current Date] | **Last Amended**: 2025-01-24
...
## 🔒 Multi-Repository Compliance (NON-NEGOTIABLE)

> **Multi-Sponsor Architecture**: System comprises public core repository + private sponsor repositories. Each has different compliance obligations.

### Public Core Repository Compliance

**Repository**: `clinical-diary` (GitHub public)

**Compliance Requirements**:
- ✅ No proprietary information
- ✅ No sponsor-specific logic or identifiers
- ✅ No authentication credentials or secrets
- ✅ No PHI/PII can be inferred from code
- ✅ All changes publicly visible (audit trail via git history)

**What CAN be in Core**:
- Abstract interfaces and base classes
- Generic database schema (record_audit, record_state, etc.)
- Core Event Sourcing pattern
- Generic RLS policies (parameterized by role/site)
- Contract tests defining sponsor obligations
- Documentation (excluding sponsor-specific details)

**What MUST NOT be in Core**:
- Sponsor names or identifiers
- EDC integration details
- Proprietary algorithms
- Sponsor-specific validation rules
- Authentication credentials (even examples)
- Sponsor-specific branding or configuration

**Validation**: Automated scan on PR:
```bash
# In core repository CI/CD
./scripts/check-no-sponsor-info.sh
# Fails if: sponsor names, credentials, proprietary code detected
```

### Private Sponsor Repository Compliance

**Repository**: `clinical-diary-{sponsor}` (GitHub private, per sponsor)

**Access Control**:
- Only sponsor organization + development team can access
- Separate repository per sponsor (no shared sponsor repos)
- GitHub private repository with restricted collaborators
- Sponsors cannot access each other's repositories
...
- EDC integration code (if proxy mode)
- Sponsor database extensions
- Edge Functions with sponsor logic
- Branding assets (logo, colors, theme)
- Site configuration and initial data

**Audit Trail for Sponsor Changes**:
```bash
# All sponsor repo changes tracked in git
git log --all --oneline database/extensions.sql
git log --all --oneline edge_functions/edc-sync/

# Deploy audit: Tag releases
git tag -a v1.2.3 -m "Production deployment 2025-01-24"
git push --tags
```

### Build System Compliance

**Core Package Publishing** (GitHub Package Registry):
```bash
# In core repository
npm version minor  # Semantic versioning
npm publish @clinical-diary/database@1.3.0

# Compliance: Published package is immutable
# Once published, version 1.3.0 cannot be modified
# Audit trail: npm registry logs
```

**Sponsor Build Validation** (21 CFR Part 11):
```bash
# In sponsor repository CI/CD

# 1. Contract Test Validation
npm test  # MUST pass all core contract tests
# Compliance: Ensures sponsor implements required interfaces

# 2. Compliance Scan
./scripts/compliance-check.sh
# Checks: Audit trail support, authentication checks, encryption

# 3. Security Scan
npm audit  # Check dependencies for vulnerabilities
./scripts/check-secrets.sh  # Ensure no hardcoded secrets

# 4. Build Artifact Signing
# Sign build output for tamper detection
openssl dgst -sha256 -sign private.key -out build.sig build/output

# 5. Deployment Approval
# Requires manual approval from authorized personnel
# Logged in deployment_log table
```

**Validation Workflow** (develop → release → production):
```
develop branch
    ↓
  (merge)
    ↓
release branch ← Create tag here
    ↓
  (UAT/validation in staging)
    ↓
  (approval ticket required)
    ↓
production deployment
    ↓
git tag v1.2.3 (immutable)
```

### Contract Testing Compliance

**Core Provides Contract Tests** (compliance obligations):
```dart
// packages/contracts/test/audit_trail_test.dart
test('All data modifications must create audit entry', () {
  // Test that sponsor implementation creates audit trail
  final auditEntry = captureAuditEntry(() {
    sponsorImplementation.createRecord(testData);
  });

  expect(auditEntry, isNotNull);
  expect(auditEntry.operation, 'USER_CREATE');
  expect(auditEntry.created_by, isNotEmpty);
  expect(auditEntry.change_reason, isNotEmpty);
});

test('Audit entries must be immutable', () {
  final auditId = createTestAuditEntry();

  // Attempt to modify should fail
  expect(
    () => sponsorImplementation.updateAuditEntry(auditId, newData),
    throwsException,
  );
});
```

**Sponsors Must Pass** (compliance verification):
```bash
# In sponsor repository
npm test

# Output:
# ✓ All data modifications must create audit entry
# ✓ Audit entries must be immutable
# ✓ Authentication required for all operations
# ✓ Authorization enforced via RLS
# ✓ Encryption enabled for sensitive data

# CI/CD: Cannot deploy if any test fails
```

### Compliance Reporting Across Repositories

**Core Repository**:
- Public commit history = audit trail
- Release tags = version control
- GitHub Package Registry = artifact management

**Sponsor Repository**:
- Private commit history = sponsor change audit
- Deployment tags = production releases
- Build artifacts signed = tamper detection
...
# Output: compliance-report-YYYY-MM-DD.pdf
# Includes:
# - Core version used: @clinical-diary/database@1.3.0
# - Sponsor extensions: database/extensions.sql (SHA256: abc123...)
# - Contract test results: 42/42 passed
# - Deployment history: v1.2.3 deployed 2025-01-24 by user@example.com
...
# Clinical Trials Compliance Requirements (from prd-clinical-trials.md)
